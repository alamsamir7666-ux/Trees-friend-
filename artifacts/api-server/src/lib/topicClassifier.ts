...
export async function classifyTopic(message: string): Promise<TopicCheckResult> {
  ...
  try {
    ...
    // ─── No LLM configured — fail-open ─────────────────────────────────────
    logger.warn("Topic classifier: no LLM configured, failing open (allowing message)");
    return {
      isOnTopic: true,
      confidence: 0,
      provider: "fail-open",
      latencyMs: Date.now() - startTime,
      explanation: "No LLM configured — message allowed (fail-open)",
    };
  } catch (err) {
