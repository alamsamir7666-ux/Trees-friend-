      ...
      await pool.query(
        `UPDATE ai_kb_entries
           SET embedding = $1::vector,
               ...
               updated_at = NOW()
         WHERE id = $2`,
        [embeddingStr, row.id, result.model],
      );
      succeeded++;
      sourceIdsToCheck.add(row.source_id);
    }
...
// ─── markStaleEmbeddingsForReembedding ──────────────────────────────────────
/**
 * BUG-E1 critical fix: marks all embeddings generated by a DIFFERENT model
 * than the current `EMBEDDING_MODEL` as 'pending' for re-embedding.
 *
 * This is called on server startup (from ensureAiTables.ts) to handle the
 * case where the operator changed `GEMINI_EMBEDDING_MODEL` env var without
 * running a migration. Any entry whose `embedding_model` doesn't match the
 * current model gets its embedding cleared + status set to 'pending' — the
 * background job will re-embed it with the new model on the next run.
 *
 * Also handles entries with `embedding IS NOT NULL` but
 * `embedding_model IS NULL` (pre-migration rows that were embedded before
 * the `embedding_model` column existed).
 *
 * Returns the number of entries marked for re-embedding.
 */
export async function markStaleEmbeddingsForReembedding(): Promise<number> {
  try {
    ...
    if (count > 0) {
      logger.info(
        { count, currentModel: EMBEDDING_MODEL },
        "KB embeddings: marked stale-model embeddings for re-embedding (BUG-E1 fix)",
      );
    }
    return count;
  } catch (err) {
    // Non-fatal — if the embedding_model column doesn't exist yet (the
    // migration hasn't run), this will fail. The migration adds the column;
    // after it runs, this function will work on the next startup.
    logger.debug(
      { err: (err as any)?.message },
      "KB embeddings: stale-model check failed (column may not exist yet — non-fatal)",
    );
    return 0;
  }
}
