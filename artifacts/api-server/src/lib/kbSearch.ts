...
export async function searchKnowledgeBase(params: {
  ...
  // BUG-E1 critical fix: only compare against embeddings generated by the
  // SAME model as the current query embedding. Embeddings from a different
  // model (e.g. old text-embedding-004 vectors) are in a different vector
  // space — cosine similarity is meaningless. We pass the current model
  // name as a parameter and use a CASE expression to zero out the semantic
  // score for entries with a stale/NULL embedding_model.
  let embeddingParamIdx: number | null = null;
  let embeddingModelParamIdx: number | null = null;
  if (queryEmbedding) {
    embeddingParamIdx = paramIdx++;
    sqlParams.push(`[${queryEmbedding.join(",")}]`);
    // Pass the current embedding model name so the SQL can filter
    // stale embeddings (embedding_model != current model → semantic = 0).
    embeddingModelParamIdx = paramIdx++;
    sqlParams.push(EMBEDDING_MODEL);
  }
  ...
  // Semantic: 1 - cosine_distance. 0 if no embedding OR stale model.
  // BUG-E1 critical fix: CASE WHEN e.embedding_model = $currentModel
  // ensures we only compute cosine similarity for same-model embeddings.
  // Stale embeddings (from a different model) get semantic = 0 and fall
  // back to keyword-only matching.
  const semanticSelect = queryEmbedding
    ? `CASE WHEN e.embedding_model = $${embeddingModelParamIdx} THEN 1 - (e.embedding <=> $${embeddingParamIdx}::vector) ELSE 0 END`
    : `0`;
  const semanticOrderBy = queryEmbedding
    ? `CASE WHEN e.embedding_model = $${embeddingModelParamIdx} THEN (1 - (e.embedding <=> $${embeddingParamIdx}::vector)) * ${WEIGHT_SEMANTIC} ELSE 0 END`
    : `0`;
  ...
  // BUG-E1 fix: semantic filter also checks embedding_model = current model
  // so stale embeddings don't produce false-positive similarity matches.
  whereClauses.push(
    `(${queryEmbedding ? `(e.embedding_model = $${embeddingModelParamIdx} AND 1 - (e.embedding <=> $${embeddingParamIdx}::vector) > 0.3) OR ` : ""}e.search_tsvector @@ ${tsQueryExpr}${keywordArrayParamIdx ? ` OR ${keywordArrayOverlapExpr} > 0` : ""})`,
  );
...
async function searchKnowledgeBaseFallback(params: {
  ...
  if (queryEmbedding) {
    embeddingParamIdx = paramIdx++;
    sqlParams.push(`[${queryEmbedding.join(",")}]`);
    // BUG-E1 fix: pass the current model name for stale-embedding filtering.
    embeddingModelParamIdx = paramIdx++;
    sqlParams.push(EMBEDDING_MODEL);
  }
  ...
  // BUG-E1 fix: CASE WHEN e.embedding_model = $model guards against
  ...
  // BUG-E1 fix: semantic filter also checks embedding_model = current model.
  whereClauses.push(
    `(${queryEmbedding ? `(e.embedding_model = $${embeddingModelParamIdx} AND 1 - (e.embedding <=> $${embeddingParamIdx}::vector) > 0.3) OR ` : ""}e.search_tsvector @@ websearch_to_tsquery('english', $${tsQueryParamIdx})${keywordArrayParamIdx ? ` OR ${keywordArrayOverlapExpr} > 0` : ""})`,
  );
