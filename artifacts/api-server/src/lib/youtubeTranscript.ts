 ...
 * bot-challenge page.
 *
 * We also tested direct HTML scraping of the watch page + the timedtext
 * endpoint — it doesn't work either, because YouTube now requires a
 * Proof-of-Origin Token (POT) generated by the player JS at runtime.
 * Without it, the timedtext endpoint returns HTTP 200 with an empty body.
 *
 * Our graceful-degradation strategy (3 tiers):
 *
 *   1. Tier 1 — Try youtubei.js with no auth (works on residential IP).
 *   2. Tier 2 — If bot-challenged AND YOUTUBE_SESSION_COOKIE env var is set,
 *      retry youtubei.js with the cookie (a real user's browser session —
 *      passes the bot check). The admin exports the cookie from their
 *      browser using a "Get cookies.txt" extension and pastes it into the
 *      env var. (Admin opt-in escape hatch.)
 *   3. Tier 3 — If all else fails (or no cookie is configured), fall back
 *      to returning ONLY the metadata (via oEmbed — works from any IP,
 *      no auth, no rate limits) + a manual-fallback reason asking the
 *      admin to upload a .vtt/.srt file (via the transcript-file route)
 *      or paste the transcript manually.
 *
 * This way:
 *   - On the admin's laptop (dev):               Tier 1 succeeds (fully automatic)
 *   - On Render with YOUTUBE_SESSION_COOKIE set:  Tier 2 succeeds (fully automatic)
 *   - On Render without cookie:                  Tier 3 metadata-only fallback
 *                                                 → admin uploads .vtt/.srt file
 *
 ...
 *   2. Tier 2 — If bot-challenged AND YOUTUBE_SESSION_COOKIE env var is set,
 *      retry Innertube with the cookie (admin opt-in escape hatch).
 *   3. Tier 3 — Fall back to oEmbed metadata only with a manual-fallback
 *      reason explaining the admin should upload a .vtt/.srt file or paste
...
export async function fetchYoutubeTranscript(url: string): Promise<YoutubeTranscriptResult> {
  ...
  try {
    ...
    if (!isBotChallengeError(err)) {
      ...
      // Log it and fall through to Tier 2 (cookie retry) as a last resort.
      logger.warn(
        { videoId, err: (err as Error).message.slice(0, 200) },
        "YouTube: Innertube (no-auth) failed with non-bot error, trying Tier 2 (cookie)",
      );
    } else {
      logger.info(
        { videoId },
        "YouTube: Innertube (no-auth) bot-challenged, trying Tier 2 (cookie) if configured",
      );
    }
  }

  // ─── Tier 2: Retry Innertube with cookie if configured ───
  const cookie = process.env.YOUTUBE_SESSION_COOKIE;
  if (cookie && cookie.trim()) {
    try {
    ...
    logger.info(
      { videoId },
      "YouTube: no YOUTUBE_SESSION_COOKIE configured, skipping Tier 2 cookie retry",
    );
  }
