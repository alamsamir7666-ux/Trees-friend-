...
//  Main Page
//
//  Dedicated route (/category/:slug/listings) showing every seller's
//  listing for the products in a leaf category, grouped by product, as
//  swipeable rows with Add to Bag. This is the "shop now" surface, linked
//  to from Shop All's "View all". The default /category/:slug route
//  (CategoryProductsPage) shows the product/variety browse list instead --
//  that's the entry point from Browse and normal category navigation.
// ══════════════════════════════════════════════════════════════════════════
...
export function CategoryListingsPage() {
  ...
  useEffect(() => {
    if (currentCat) {
      updateSEO({
        title: `${currentCat.name} sellers — Tree Friend`,
        description: `Compare seller listings, prices, and availability for ${currentCat.name} on Tree Friend.`,
      });
    }
  }, [currentCat]);
  ...
  const crumbs = [
    ...
    { label: currentCat.name, href: `/category/${currentCat.slug}` },
    { label: "Sellers" },
  ];
  ...
  return (
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        {isParentCategory ? (
          // ── Parent category: this route only applies to leaf categories.
          // "Fruit Tree" (a parent) has no seller listings of its own --
          // point the shopper at its subcategories instead.
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center mb-4">
              <Sprout className="w-7 h-7 text-muted-foreground" />
            </div>
            <h2 className="font-serif text-xl font-medium mb-2">Pick a variety first</h2>
            <p className="text-sm text-muted-foreground max-w-[360px] mb-6">
              {currentCat.name} has {subcategories.length} subcategor{subcategories.length === 1 ? "y" : "ies"} -- browse those to see seller listings.
            </p>
            <Link href={`/category/${currentCat.slug}`}>
              <Button className="rounded-full">Browse {currentCat.name}</Button>
            </Link>
          </div>
        ) : listingsLoading ? (
          // ── Loading skeletons ──────────────────────────────────────────
