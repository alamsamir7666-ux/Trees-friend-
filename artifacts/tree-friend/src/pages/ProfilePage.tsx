  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:flex lg:max-w-7xl lg:mx-auto">
        ...
        <aside className="bg-card border-b lg:border-b-0 lg:border-r lg:w-72 lg:shrink-0 lg:sticky lg:top-0 lg:self-start">
          <div className="container mx-auto px-4 md:px-6 lg:px-6 pt-6 pb-8 max-w-5xl lg:max-w-none lg:pt-8 lg:pb-6">
            <PageBreadcrumb crumbs={[{ label: "My Profile", icon: <Star className="h-3 w-3" /> }]} className="mb-5 lg:mb-4" />
            ...
            <div className="flex items-start gap-5 lg:flex-col lg:items-center lg:gap-0">
              ...
              {/* Name & details */}
              <div className="flex-1 min-w-0 pt-1 lg:pt-4 lg:text-center">
          ...
          <nav className="hidden lg:block px-6 pb-6 space-y-1">
            <button
              onClick={() => setProfileTab("overview")}
              className={`w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-2.5 ${profileTab === "overview" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
            >
              <Package2 className="h-4 w-4" />Overview
            </button>
            <button
              onClick={() => {
              ...
              className={`w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-2.5 ${profileTab === "seller" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
            >
            ...
            <button
              ...
              className={`w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-2.5 ${profileTab === "rewards" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
            >
        ...
        <div className="flex-1 min-w-0">
          <div className="container mx-auto px-4 md:px-6 lg:px-10 py-6 max-w-5xl lg:max-w-none lg:py-8">
            {/* ── Profile Tabs (mobile only, hidden on lg+) ─────────────── */}
            <Tabs value={profileTab} onValueChange={setProfileTab} className="mb-6 lg:hidden">
            ...
            <div className="space-y-6 lg:space-y-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 lg:gap-5">
                {quickActions.map(({ label, icon: Icon, href, count }) => (
                  <Link key={label} href={href}>
                    <div className="bg-card rounded-2xl border border-border p-4 lg:p-6 hover:shadow-md transition-all duration-200 cursor-pointer group text-center">
                      <div className="h-10 w-10 lg:h-12 lg:w-12 rounded-xl bg-muted group-hover:bg-accent/10 flex items-center justify-center mx-auto mb-2 transition-colors">
                        <Icon className="h-5 w-5 lg:h-6 lg:w-6 text-muted-foreground group-hover:text-accent transition-colors" />
                      </div>
                      <p className="text-sm lg:text-base font-medium text-foreground">{label}</p>
                      {count !== undefined && count > 0 && (
                        <p className="text-xs lg:text-sm text-muted-foreground/70 mt-0.5">{count} order{count !== 1 ? "s" : ""}</p>
                      )}
                    </div>
                  </Link>
                ))}
              ...
              <div className="bg-card rounded-2xl border border-border p-5 lg:p-8">
                <div className="flex items-center justify-between mb-4 lg:mb-5">
                  <div>
                    <h2 className="font-semibold text-foreground text-sm lg:text-lg">Following</h2>
                    {!followedSellersLoading && (followedSellers?.length ?? 0) > 0 && (
                      <p className="text-xs lg:text-sm text-muted-foreground/70 mt-0.5">{followedSellers!.length} {followedSellers!.length === 1 ? "store" : "stores"}</p>
                    )}
                  </div>
                  {(followedSellers?.length ?? 0) > 0 && (
                    <span className="text-xs lg:text-sm text-muted-foreground/70">Tap to visit</span>
                  )}
                </div>
                {followedSellersLoading ? (
                  ...
                  <div className="flex gap-3 lg:gap-4 overflow-x-auto pb-1 -mx-1 px-1">
                    {followedSellers!.map((s) => (
                      <Link key={s.id} href={`/store/${s.id}`} className="shrink-0 w-44 lg:w-52">
                        <div className="bg-muted rounded-xl p-3.5 lg:p-4 h-full hover:bg-muted/70 transition-colors border border-border">
                          <div className="flex items-center gap-2.5 mb-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1">
                                <p className="text-sm lg:text-base font-semibold text-foreground truncate">{s.nurseryName}</p>
                                {s.isVerified && <ShieldCheck className="h-3.5 w-3.5 text-warning-foreground shrink-0" />}
                              </div>
                              {s.location && (
                                <p className="text-xs lg:text-sm text-muted-foreground/70 truncate mt-0.5 flex items-center gap-1">
              ...
              <div className="bg-card rounded-2xl border border-border p-5 lg:p-8">
                <div className="flex items-center justify-between mb-4 lg:mb-5">
                  <div>
                    <h2 className="font-semibold text-foreground text-sm lg:text-lg">Recent Orders</h2>
                    <p className="text-xs lg:text-sm text-muted-foreground/70 mt-0.5">{orders?.length ?? 0} total orders</p>
                  </div>
                  <Link href="/orders">
                    <span className="text-xs lg:text-sm text-accent hover:underline flex items-center gap-1 font-medium">
                  ...
                  <div className="space-y-2.5 lg:space-y-3">
                    {allRecent.map((order: any) => {
                      ...
                      <Link key={isPreOrder ? `pre-${order.id}` : order.id} href={href}>
                        <div className="flex items-center gap-4 px-4 lg:px-5 py-3.5 lg:py-4 rounded-xl bg-muted hover:bg-muted/70 transition-colors cursor-pointer group border border-border">
                          <div className={`h-10 w-10 lg:h-11 lg:w-11 rounded-xl ${statusCfg.bg} flex items-center justify-center shrink-0`}>
                            <StatusIcon className={`h-4.5 w-4.5 lg:h-5 lg:w-5 ${statusCfg.text}`} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              {isPreOrder && (
                                <span className="text-[10px] lg:text-xs font-bold bg-info text-info-foreground rounded-full px-1.5 py-0.5">PRE-ORDER</span>
                              )}
                              <p className="text-sm lg:text-base font-medium text-foreground">{label}</p>
                            </div>
                            <p className="text-xs lg:text-sm text-muted-foreground/70 mt-0.5">{new Date(order.createdAt).toLocaleDateString()}</p>
                          </div>
                          <div className="text-right shrink-0">
                            <p className="text-sm lg:text-base font-semibold text-foreground">Tk{Number(total).toLocaleString()}</p>
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] lg:text-xs font-semibold ${statusCfg.bg} ${statusCfg.text} capitalize`}>
                              {status}
                            </span>
                          </div>
              ...
              <div className="bg-card rounded-2xl border border-border overflow-hidden" id="settings">
                <div className="px-5 lg:px-8 py-4 lg:py-5 border-b border-border/70">
                  <h2 className="font-semibold text-foreground text-sm lg:text-lg">Account Settings</h2>
                  <p className="text-xs lg:text-sm text-muted-foreground/70 mt-0.5">Manage your profile, email, and connected accounts</p>
                </div>

                {/* Profile details row */}
                <div className="px-5 lg:px-8 py-4 border-b border-border/70 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs lg:text-sm text-muted-foreground/70 uppercase tracking-wider font-medium">Profile</p>
                    <p className="text-sm lg:text-base font-medium text-foreground truncate">{user?.fullName ?? "Your Profile"}</p>
                  </div>
                  <Link href="/profile#settings">
                    <span className="text-xs lg:text-sm text-accent hover:underline font-medium cursor-pointer">Update profile</span>
                  </Link>
                </div>

                {/* Email addresses */}
                <div className="px-5 lg:px-8 py-4 border-b border-border/70">
                  <p className="text-xs lg:text-sm text-muted-foreground/70 uppercase tracking-wider font-medium mb-3">Email addresses</p>
                  {user?.emailAddresses.map((email) => (
                    <div key={email.id} className="flex items-center gap-3 mb-2 last:mb-0">
                      ...
                      <span className="text-sm lg:text-base text-foreground flex-1 truncate">{email.emailAddress}</span>
                      {email.id === user.primaryEmailAddressId && (
                        <span className="text-[10px] lg:text-xs font-medium bg-muted text-muted-foreground px-2 py-0.5 rounded-full">Primary</span>
                      )}
                    </div>
                  ))}
                </div>
                ...
                {/* Connected accounts */}
                <div className="px-5 lg:px-8 py-4 border-b border-border/70">
                  <p className="text-xs lg:text-sm text-muted-foreground/70 uppercase tracking-wider font-medium mb-3">Connected accounts</p>
                  {user?.externalAccounts && user.externalAccounts.length > 0 ? (
                    user.externalAccounts.map((acc: any) => (
                      <div key={acc.id} className="flex items-center gap-3 mb-2 last:mb-0">
                        ...
                        <span className="text-sm lg:text-base text-foreground flex-1 truncate capitalize">{acc.provider} &middot; {acc.emailAddress ?? acc.username ?? "Connected"}</span>
                      </div>
                    ))
                  ) : (
