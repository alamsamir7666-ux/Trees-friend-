  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-5 flex flex-col gap-3 hover:shadow-md transition-shadow duration-200">
      ...
      <div className="flex items-center gap-1.5">
        ...
        {change && (
          <span className={`text-xs font-medium ${trend === "up" ? "text-success-foreground" : trend === "down" ? "text-destructive" : "text-muted-foreground"}`}>
            {change}
          </span>
        )}
    ...
    <div className="bg-card rounded-xl border shadow-lg px-4 py-3 text-xs">
      ...
      {payload.map((p: any, i: number) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <span className="font-semibold text-foreground">{p.name === "revenue" ? `Tk${p.value.toLocaleString()}` : p.value}</span>
        </div>
      ))}
    </div>
  );
...
function OrderFunnel({ orders }: { orders: any[] }) {
  ...
  const statusColors: Record<string, string> = {
    pending: "bg-warning-foreground", confirmed: "bg-info-foreground", processing: "bg-info-foreground",
    shipped: "bg-info-foreground", delivered: "bg-success-foreground", cancelled: "bg-destructive",
  };
  const statusTextColors: Record<string, string> = {
    pending: "text-warning-foreground", confirmed: "text-info-foreground", processing: "text-info-foreground",
    shipped: "text-info-foreground", delivered: "text-success-foreground", cancelled: "text-destructive",
  };
    ...
    <div className="space-y-6">
        ...
        <div className="lg:col-span-2 bg-card rounded-2xl border border-border p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground/70 mt-0.5">Last 30 days, delivered orders only</p>
            </div>
            <div className="flex items-center gap-1.5">
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground/70 mt-0.5">{orders.length} total orders</p>
            </div>
            <button onClick={() => setActiveTab("orders")} className="text-xs text-primary hover:underline flex items-center gap-1">
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground/70 mt-0.5">{totalSellers} registered sellers</p>
            </div>
            <button onClick={() => setActiveTab("sellers")} className="text-xs text-primary hover:underline flex items-center gap-1">
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground/70 mt-0.5">By delivered revenue, from current orders</p>
            </div>
            <button onClick={() => setActiveTab("sellers")} className="text-xs text-primary hover:underline flex items-center gap-1">
              Manage <ChevronRight className="h-3 w-3" />
            </button>
          ...
          {topSellers.length > 0 ? (
            <div className="space-y-2">
              {topSellers.map((s, idx) => (
                <div key={s.businessName} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-primary/5 transition-colors">
                  <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 text-xs font-bold ${
                    idx === 0 ? "bg-warning text-warning-foreground" : idx === 1 ? "bg-muted text-muted-foreground" : idx === 2 ? "bg-warning/70 text-warning-foreground" : "bg-muted/50 text-muted-foreground/70"
                  }`}>
                    {idx + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium text-foreground truncate">{s.businessName}</p>
                      {s.status && s.status !== "active" && (
                        <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium border ${SELLER_STATUS_STYLE[s.status] ?? "bg-muted text-muted-foreground border-border"}`}>
                          {SELLER_STATUS_LABEL[s.status] ?? s.status}
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground/70">{s.orderCount} order{s.orderCount === 1 ? "" : "s"}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-semibold text-foreground">Tk{s.revenue.toLocaleString()}</p>
                    <p className="text-[10px] text-muted-foreground/70 uppercase tracking-wider">delivered</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {recentCombined.map((o) => {
                ...
                return (
                  <div key={o.id} className="flex items-center gap-4 px-5 py-3 hover:bg-muted/40 transition-colors">
                        ...
                        {sellerName && (
                          <>
                            ...
                            {sellerStatus && sellerStatus !== "active" && (
                              <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium border ${SELLER_STATUS_STYLE[sellerStatus] ?? "bg-muted text-muted-foreground border-border"}`}>
                    ...
                    <span className="text-sm font-semibold text-foreground shrink-0">Tk{o.totalAmount.toLocaleString()}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground/70 mt-0.5">{products.length} total products</p>
            </div>
            <button onClick={() => setActiveTab("products")} className="text-xs text-primary hover:underline flex items-center gap-1">
              Manage <ChevronRight className="h-3 w-3" />
            </button>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Pending Orders", value: pendingOrders, icon: Clock, color: "bg-warning text-warning-foreground", tab: "orders" },
          { label: "Active Orders", value: activeOrdersCount, icon: Activity, color: "bg-info text-info-foreground", tab: "orders" },
