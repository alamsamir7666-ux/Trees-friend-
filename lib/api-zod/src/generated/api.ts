...
export const listRelatedProductsQueryLimitDefault = 4;
export const listRelatedProductsQueryLimitMax = 12;



export const ListRelatedProductsQueryParams = zod.object({
  "limit": zod.coerce.number().min(1).max(listRelatedProductsQueryLimitMax).default(listRelatedProductsQueryLimitDefault).describe('Max number of related products to return (default 4, max 12).')
})
...
export const UpdateMeBody = zod.object({
  "firstName": zod.string().nullish().describe('User\'s first name. Nullable so clients can clear it\n(e.g. Clerk sign-ups without a first name).\n'),
  "lastName": zod.string().nullish().describe('User\'s last name. Nullable so clients can clear it.\n'),
  "phone": zod.string().nullish().describe('Bangladeshi mobile number in local format (01XXXXXXXXX).\nValidated server-side against isValidBdPhone().\n'),
  "email": zod.string().email().nullish().describe('Optional email override. The backend ignores emails ending\nin \"@clerk.user\" (Clerk\'s synthetic identifier for\nOAuth-only accounts) and only persists a real email address.\nFrontend `ProfileSync` always sends the current Clerk\nprimary email here so the DB row stays in sync.\n')
})
