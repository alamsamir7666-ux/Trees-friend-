...
export const preOrdersTable = pgTable(
  ...
  (table) => [
    // P0-2: index on productId — notifyPreOrderCustomers in routes/preOrders.ts
    // queries WHERE product_id = ? AND status = 'pending' AND payment_status = '...'
    // to find every buyer waiting on a given product when restock happens.
